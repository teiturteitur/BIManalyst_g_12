...
def choose_ifc_pair_from_directory(
    console: Console, directory: str, extension=".ifc"
    ...
    if not files:
        console.print(
            f"[yellow]⚠️ No {extension} files found in '{directory}'.[/yellow]"
        )
        console.print(
            f"[yellow]Please add IFC files ending with '-MEP.ifc' and '-ARCH.ifc' to {directory} and try again.[/yellow]"
        )
        sys.exit(1)
    ...
    # Display in a table
    table = Table(
        title=f"Available {extension.upper()} File Pairs in '{directory}'",
        show_lines=True,
    )
    ...
    for i, prefix in enumerate(prefixes, start=1):
        ...
        arch = (
            "✅ " + groups[prefix]["ARCH"] if "ARCH" in groups[prefix] else "❌ Missing"
        )
        table.add_row(str(i), prefix, mep, arch)

    console.print(
        "\n[bold yellow]Please make sure to select a MEP/ARCH file pair or just an ARCH file from the list below:[/bold yellow]"
    )
    ...
    while True:
        choice = Prompt.ask(
            "[bold cyan]Enter the number or prefix of the desired file[/bold cyan]"
        ).strip()
        ...
        if choice.isdigit():
            ...
            if 1 <= index <= len(prefixes):
                ...
                console.print(
                    "[red]Invalid number. Please choose one of the listed options.[/red]"
                )
                continue
        else:
            ...
            if matching:
                ...
                console.print(
                    "[red]Invalid input. Please enter a valid number or prefix name.[/red]"
                )
                continue

        # if arch file missing, check if spaces are in mep file
        if "ARCH" not in groups[selected_prefix]:
            mep_file_path = os.path.join(
                directory, groups[selected_prefix].get("MEP", "")
            )
            mep_ifc = ifcopenshell.open(mep_file_path)
            spaces = mep_ifc.by_type("IfcSpace")
            if not spaces:
                console.print(
                    f"[red]The selected ARCH file is missing and no IfcSpaces found in the MEP file '{groups[selected_prefix].get('MEP', '')}'. Please select a different pair.[/red]"
                )
                continue
        if "MEP" not in groups[selected_prefix]:
            console.print(
                f"[red]No MEP file was selected. Not all analysis will be available.[/red]"
            )
            mep_path = None
...
def choose_ifcElementType(
    console: Console, ifcFile: ifcopenshell.file, category="MEP-HVAC"
) -> tuple[list, Table]:
    ...
    if not targetElements:
        console.print(
            f"[yellow]⚠️ No elements of category '{category}' found in the IFC file.[/yellow]"
        )
        sys.exit(1)

    # Display present types and missing types in a Rich table and show how many there are of each type
    table = Table(
        title=f"Available Element Types in Category '{category}'", show_lines=True
    )
    table.add_column("#", justify="right", style="cyan", no_wrap=True)
    ...
    table.add_row(
        "",
        "[bold yellow]Total Elements Found[/bold yellow]",
        f"[bold yellow]{len(targetElements)}[/bold yellow]",
    )
    # console.print()
    # console.print(table)
    # console.print()
    #
    return targetElements, table


def copy_space_with_full_metadata(
    source_space: ifcopenshell.entity_instance, target_ifc: ifcopenshell.file
...
def merge_spaces_with_quantities_and_structure(
    console: Console, source_ifc: ifcopenshell.file, target_ifc: ifcopenshell.file
) -> tuple[ifcopenshell.file, list]:
