        ...
        if (
            ...
            for spaceID in spaceTerminals.keys():
                if pathTerminal in [
                    ...
                    terminalPset = ifcopenshell.api.pset.add_pset(
                        file=ifc_file,
                        product=ifc_file.by_guid(pathTerminal),
                        name="Pset_AirTerminalOccurence",
                    )
                    ifcopenshell.api.pset.edit_pset(
                        file=ifc_file,
                        pset=terminalPset,
                        properties={"AirFlowRate": requiredAirFlow},
                    )
...
def buildErrorDict(
    missingAHUsystems: dict, unassignedTerminals: dict, ifc_file: ifcopenshell.file
) -> dict:
    """
    ...
    for sys_name, info in missingAHUsystems.items():
        system_elements = [
            ifc_file.by_id(el_id) for el_id in info.get("ElementIDs", [])
        ]
        ...
        elements_dict[f"Missing AHU - {sys_name}"] = [
            {"element": system_elements, "message": msg}
        ]
    ...
    for flow_dir, element_ids in unassignedTerminals.items():
        ...
        for el_id in element_ids:
            ...
            msg = (
                f"Air terminal {el_id} ({flow_dir}) is not located inside an IfcSpace."
            )
